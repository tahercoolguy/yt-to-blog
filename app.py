...
def generate_blog(yt_url):
    ...
    if total_length > 2500:
        while True:
            if total_length > 2500 :
                new_prompt = "summaries given text in 10 points with very less description ' "+main_transcript[completed_length:completed_length+2500] +" '"
                new_data = chat_model.predict(new_prompt)
                final_transcript = final_transcript + new_data
                completed_length = completed_length + 2500
